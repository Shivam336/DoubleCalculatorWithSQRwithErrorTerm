...
public class BusinessLogic {
	...
	**********************************************************************************************/
	...
	public boolean setOperand1et(String value) {
		...
		operand1et = new CalculatorValue(value);			// If there was input text, try to convert it
		operand1ErrorMessage = operand1et.getErrorMessage();	// into a CalculatorValue and see if it
		if (operand1ErrorMessage.length() > 0) 			// worked. If there is a non-empty error 
			return false;								// message, signal there was a problem.
	...
	public boolean setOperand2et(String value) {
		operand2etDefined = false;							// Assume the operand will not be defined
		if (value.length() <= 0) {						// See if the input is empty. If so no error
			operand2ErrorMessage = "";					// message, but the operand is not defined.
			return true;									// Return saying there was no error.
		}
		operand2et = new CalculatorValue(value);			// If there was input text, try to convert it
		operand2ErrorMessage = operand2et.getErrorMessage();	// into a CalculatorValue and see if it
		if (operand2ErrorMessage.length() > 0) 			// worked. If there is a non-empty error 
			return false;								// message, signal there was a problem.
		operand2etDefined = true;							// Otherwise, set the defined flag and
		return true;										// signal that the set worked
	}
	...
	/**********
	 * This public getter fetches the defined attribute for operand1. We can't use the lack of an error 
	...
	/**********
	 * This public getter fetches the defined attribute for operand2. We can't use the lack of an error 
	 * message to know that the operand is ready to be used. An empty operand has no error associated with 
	 * it, so the class checks to see if it is defined and has no error before setting this flag true.
	 * 
	 * @return true if the operand is defined and has no error, else false
	...
	**********************************************************************************************/
	...
	// Control the Divide by zero error and show error message when quotient shows Infinity    
